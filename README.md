...
補足: Sentinel-2 は `s2:processing_baseline >= 4.0` のシーンを自動判定し、`output/sentinel2/img/` 保存時に DN から 1000 を差し引いた補正済み DN として出力します（ESA の RADIO_ADD_OFFSET 対応）。1/10000 の反射率変換は cloudmask / masked / snowmasked などの後段で適用します。GeoTIFF タグとメタデータ GeoJSON には、判定に使った processing baseline と適用した DN add offset を保存します。
